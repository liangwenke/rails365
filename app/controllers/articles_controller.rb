class ArticlesController < ApplicationController
  ...
  def index
    if params[:search].present?
      @articles = Article.published.select("title, created_at, published, group_id, slug, id").search_by_title_or_body(params[:search]).order("id DESC").page(params[:page])
    else
  ...
  def show
    ...
    @recommend_articles = Article.published.select("title, created_at, published, group_id, slug, id").search_by_title_or_body(@group_name).order("visit_count DESC").limit(10)
  end
